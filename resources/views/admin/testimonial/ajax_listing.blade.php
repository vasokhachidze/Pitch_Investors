...
<tr align="center" class="text-center">
    ...
    <td class="white-space">@if(!empty($value->vName))
      {{$value->vName}}
	  @else
	  {{"N/A"}}
	  @endif</td>
	  <td style="width: 45%;">@if(!empty($value->tDescription))
      {!!$value->tDescription!!}
	  @else
	  {{ "N/A" }}
	  @endif</td>
	...
	<td class="white-space">
			<a class="btn btn-info btn-sm" href="{{url('admin/testimonial/edit',$value->vUniqueCode)}}"><i class="fas fa-pencil-alt"></i></a>
