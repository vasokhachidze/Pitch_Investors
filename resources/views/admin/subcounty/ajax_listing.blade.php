...
<tr align="center" class="text-center">
	...
	<td class="white-space">@if(!empty($value->countyName))
		{{$value->countyName}}
		@else
		{{ "N/A" }}
		@endif
	</td>
	<td class="white-space">@if(!empty($value->regionName))
      {{$value->regionName}}
	  @else
	  {{ "N/A" }}
	  @endif
	...
	<td class="white-space">
			<a class="btn btn-info btn-sm" href="{{url('admin/subCounty/edit',$value->vUniqueCode)}}"><i class="fas fa-pencil-alt"></i></a>
