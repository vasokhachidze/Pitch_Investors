...
<tr align="center" class="text-center">
	...
	<td>
	  @if(!empty($value->vCountry))
      {{$value->vCountry}}
	  @else
	  {{ "N/A" }}
	  @endif
	</td>
	<td>@if(!empty($value->vCountryCode))
      {{$value->vCountryCode}}
	  @else
	  {{ "N/A" }}
	  @endif
	</td>
	<td>@if(!empty($value->vCountryISDCode))
      {{$value->vCountryISDCode}}
	  @else
	  {{ "N/A" }}
	  @endif
	</td>
	...
	<td>
			<a class="btn btn-info btn-sm" href="{{url('admin/country/edit',$value->vUniqueCode)}}"><i class="fas fa-pencil-alt"></i></a>
