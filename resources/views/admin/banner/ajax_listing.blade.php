...
<tr align="center" class="text-center">
    ...
    <td>
        @if(!empty($value->vBannerImage))
        <img alt="{{$value->vBannerImage}}" class="img-fluid" style="height: 35px;" src="{{asset('uploads/banner/'.$value->vBannerImage)}}"></td>
        @else
        <img alt="no-image" class="img-fluid" style="height: 35px;" src="{{asset('uploads/default/no-image.png')}}"></td>
        @endif
	<td class="white-space">
        @if(!empty($value->vBannerTitle))
            {{$value->vBannerTitle}}
	...
	<td>
        @if(!empty($value->vSubTitle))
      
        {{ strtolower(trans($value->vSubTitle))}}
        @else
        {{ "N/A" }}
        @endif
    </td>
	...
	<td class="white-space">
		<a class="btn btn-info btn-sm" href="{{url('admin/banner/edit',$value->vUniqueCode)}}"><i class="fas fa-pencil-alt"></i></a>
